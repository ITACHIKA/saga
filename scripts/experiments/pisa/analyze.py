...
def load_results(resultspath: pathlib.Path) -> pd.DataFrame:
    ...
    """
    from saga.pisa.simulated_annealing import SimulatedAnnealingRun
    ...
    for run_dir in resultspath.glob("*_vs_*"):
        ...
        try:
            # Load via SimulatedAnnealingRun to compute best_iteration
            run = SimulatedAnnealingRun.model_validate_json(run_json_path.read_text())
            makespan_ratio = run.best_iteration.current_energy
            ...
            logging.warning(f"Failed to load {run_dir}: {e}")
            continue

    df_results = pd.DataFrame(
        rows, columns=["Base Scheduler", "Scheduler", "Makespan Ratio"]
    )
    return df_results
...
def results_to_csv(resultspath: pathlib.Path, outputpath: pathlib.Path):
...
def tab_results(
    ...
    rename_dict = {"CPOP": "CPoP", "Fastest Node": "FastestNode", **SCHEDULER_RENAMES}
    rename_dict = {
        **rename_dict,
    ...
    df_all_results["Base Scheduler"] = df_all_results["Base Scheduler"].replace(
        rename_dict
    )
    ...
    if include_hybrid:
        ...
        for scheduler in df_results["Scheduler"].unique():
            ...
            try:
                hybrid_values.append(
                    [scheduler, "Hybrid", res["Makespan Ratio"].values[0]]
                )
            except IndexError:
                pass
        ...
        df_hybrid = pd.DataFrame(
            hybrid_values, columns=["Scheduler", "Base Scheduler", "Makespan Ratio"]
        )
    ...
    def default_order(x: str) -> str:
        return (
            x.replace("Hybrid", "ZHybrid")
            .replace("Worst", "ZWorst")
            .replace(r"\textit", "AA")
        )
    ...
    if mode is None or mode == "pdf":
        fig = axis.get_figure()
        if fig is not None:
            fig.savefig(savedir / f"{savename}.pdf", dpi=300, bbox_inches="tight")
    if mode is None or mode == "png":
        fig = axis.get_figure()
        if fig is not None:
            fig.savefig(savedir / f"{savename}.png", dpi=300, bbox_inches="tight")
...
def main():
    ...
    tab_results(outputdir, outputdir, mode="png")
